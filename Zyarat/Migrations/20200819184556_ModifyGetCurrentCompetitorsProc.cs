...
{
    public partial class ModifyGetCurrentCompetitorsProc : Migration
    {
...
@MinUniqueVisits AS INT 
    AS 
SELECT ROW_NUMBER() OVER (ORDER BY COUNT(DISTINCT e.EvaluatorId) DESC,MAX(v.DateTime) DESC,MIN(e.DateTime)) AS Ranking
